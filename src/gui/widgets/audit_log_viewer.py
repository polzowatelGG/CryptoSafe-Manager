...
class AuditLogViewer(QWidget):
    ...
    def _build_filter_panel(self) -> QWidget:
        ...
        self.search_input.setPlaceholderText("Поиск по деталям / типу события...")
        ...
        self.type_filter.addItems([
            "Все типы",
            "SYSTEM_GENESIS", "USER_LOGIN", "USER_LOGOUT", "LOGIN_FAILED",
            ...
            "PANIC_MODE_ACTIVATED", "VAULT_LOCKED", "VAULT_UNLOCKED",
        ])
    ...
    def _apply_filters(self):
        ...
        for row in self._all_entries:
            ...
            if search:
                ...
                # Ищем в деталях — сериализуем весь details dict в строку
                details_str = json.dumps(
                    data.get('details', {}), ensure_ascii=False
                ).lower()
                # Ищем в source и user_id
                source_str = str(data.get('source', '')).lower()
                user_str   = str(data.get('user_id', '')).lower()

                haystack = (
                    event_type_str + " " + details_str
                    + " " + source_str + " " + user_str
                )
    ...
    def _render_page(self):
        ...
        for row_idx, row in enumerate(page_entries):
            ...
            brush    = QBrush(sev_colors.get(severity, sev_colors['INFO']))
    ...
    def _show_context_menu(self, pos):
        ...
        if event_type in ('ENTRY_CREATED', 'ENTRY_UPDATED', 'ENTRY_DELETED'):
            entry_id = data.get('details', {}).get('entry_id', '')
            if entry_id:
                goto = QAction("🔍 Перейти к записи в хранилище", self)
                goto.triggered.connect(
                    lambda: self.vault_entry_selected.emit(entry_id)
                )
                menu.addAction(goto)
        detail_action = QAction("📋 Показать детали", self)
        detail_action.triggered.connect(lambda: self._show_entry_details(row))
