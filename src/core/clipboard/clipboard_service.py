...
class ClipboardService: # основной сервис для управления буфером обмена, который взаимодействует с платформой и системой событий, а также с монитором изменений буфера обмена
    ...
    def update_timeout(self, new_timeout: int):
        """Обновляет таймаут и перезапускает таймер если буфер сейчас активен."""
        import threading
        from datetime import datetime

        with self._lock:
            # Обновляем конфиг — следующие копирования используют новое значение
            # config.set_preference уже вызван снаружи, здесь просто перезапускаем таймер

            if not self._current_content:
                return  # буфер пуст — ничего перезапускать не нужно
            ...
            # Считаем сколько времени уже прошло
            elapsed = (datetime.utcnow() - self._current_content.copied_at).total_seconds()
            remaining = max(1, new_timeout - elapsed)

            op_id = self._operation_id

            # Запускаем новый таймер с актуальным таймаутом
